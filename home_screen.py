...
def show_home_screen(window ,start_game_callback , start_bot_game_callback):
    styles = setup_styles()
    clear_window(window)
    ...
    Button(frame, text="Play vs Bot", font=styles["fontLarge"], bg=styles["buttonColor"], fg=styles["buttonFgColor"], command=lambda: play_vs_bot(start_bot_game_callback)).grid(row=1, column=0, sticky="ew", padx=50)
    Button(frame, text="Play 2 Players on this Computer", font=styles["fontLarge"], bg=styles["buttonColor"], fg=styles["buttonFgColor"], command=lambda: play_local_multiplayer(start_game_callback)).grid(row=2, column=0, sticky="ew", padx=50, pady=10)
